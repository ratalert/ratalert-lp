...
contract("StakingRewardPool", (accounts) => {
    ...
    before(async () => {
        ...
        const daoBalance = await cakeLP.balanceOf(dao);
        await cakeLP.transfer(accounts[0], daoBalance, { from: dao });
    });
    ...
    it("starting a new reward phase should setup the reward phase and reward token balance in the pool", async () => {
        ...
        await etb.approve(pool.address, reward, { from: dao });

        // start a new reward phase
        await pool.newRewardPeriod(reward, start, end, { from: dao });
    ...
    it("claimableReward should return the reward yet to be claimed ", async () => {
        ...
        await etb.approve(pool.address, periodReward, { from: dao });
